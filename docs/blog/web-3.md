---
link: null
title: 浏览器重绘(repaint)重排(reflow)与优化[浏览器机制]
description: 很多人都知道要减少浏览器的重排和重绘，但对其中的具体原理以及如何具体操作并不是很了解，当突然提起这个话题的时候，还是会一脸懵逼。希望大家可以耐着性子阅读本文，仔细琢磨，彻底掌握这个知识点！ 博客、前端积累文档、公众号、GitHub 网页生成过程： HTM
keywords: 前端,浏览器
author: null
date: 2018-12-24T02:04:41.627Z
publisher: 掘金
stats: paragraph=71 sentences=73, words=407
---
# 重绘(repaint)重排(reflow)与优化

转自：[浏览器重绘(repaint)重排(reflow)与优化[浏览器机制]](https://juejin.im/post/5c15f797f265da61141c7f86)
...
![浏览器机制](https://user-gold-cdn.xitu.io/2018/12/16/167b642d014afaf1?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
* 隐藏要操作的dom 在要操作dom之前，通过display隐藏dom，当操作完成之后，才将元素的display属性为可见，因为不可见的元素不会触发重排和重绘。
...
* 可以把动画效果应用到position属性为absolute或fixed的元素上，这样对其他元素影响较小 动画效果还应牺牲一些平滑，来换取速度，这中间的度自己衡量： 比如实现一个动画，以1个像素为单位移动这样最平滑，但是reflow就会过于频繁，大量消耗CPU资源，如果以3个像素为单位移动则会好很多。
* 启用GPPU加速 此部分来自[优化CSS重排重绘与浏览器性能](https://link.juejin.im?target=http%3A%2F%2Fcaibaojian.com%2Fcss-reflow-repaint.html)

**GPU(图像加速器)：** GPU 硬件加速是指应用 GPU 的图形性能对浏览器中的一些图形操作交给 GPU 来完成，因为 GPU 是专门为处理图形而设计，所以它在速度和能耗上更有效率。 GPU 加速通常包括以下几个部分：Canvas2D，布局合成, CSS3转换（transitions），CSS3 3D变换（transforms），WebGL和视频(video)。
...
[网页性能管理详解](https://link.juejin.im/?target=http%3A%2F%2Fwww.ruanyifeng.com%2Fblog%2F2015%2F09%2Fweb-page-performance-in-depth.html)

[优化CSS重排重绘与浏览器性能](https://link.juejin.im/?target=http%3A%2F%2Fcaibaojian.com%2Fcss-reflow-repaint.html)